CREATE SCHEMA IF NOT EXISTS TwitterEffectPM6;
...
CONSTRAINT fk_StockCompanyDailySummary_CompanyTicker FOREIGN KEY (CompanyTicker)
	REFERENCES StockCompanies(CompanyTicker)
    ON UPDATE CASCADE ON DELETE SET NULL,
CONSTRAINT fk_StockCompanyDailySummary_SummaryDate FOREIGN KEY (SummaryDate)
	REFERENCES CalendarDates(CalendarDate)
    ON UPDATE CASCADE ON DELETE SET NULL        
);
...
CONSTRAINT fk_IndexDailySummary_IndexTicker FOREIGN KEY (IndexTicker)
    ...
    ON UPDATE CASCADE ON DELETE SET NULL,
CONSTRAINT fk_IndexDailySummary_SummaryDate FOREIGN KEY (SummaryDate)
	REFERENCES CalendarDates(CalendarDate)
    ON UPDATE CASCADE ON DELETE SET NULL    
);
...
CONSTRAINT fk_Tweets_PersonName FOREIGN KEY (PersonName)
	REFERENCES Person(PersonName)
    ON UPDATE CASCADE ON DELETE SET NULL,
CONSTRAINT fk_Tweets_TweetDate FOREIGN KEY (TweetDate)
	REFERENCES CalendarDates(CalendarDate)
    ON UPDATE CASCADE ON DELETE SET NULL        
);
